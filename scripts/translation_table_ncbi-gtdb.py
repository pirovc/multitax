...
ncbi_tax = NcbiTx(
    version="20250907",
    urls="https://data.gtdb.ecogenomic.org/releases/release232/232.0/auxillary_files/taxdump_20250907.tar.gz",
)
ncbi_tax.filter(
    ["2", "2157"], desc=True
)  # Filter NCBI bacteria and archaea only to match GTDB
...
for r in gtdb_tax._standard_ranks:
    ...
    for leaf in ncbi_tax.nodes_rank(r):
        ...
        if tr:
            ...
            cr = gtdb_tax.rank(
                gtdb_tax.closest_parent(lca, ranks=["root"] + gtdb_tax._standard_ranks)
            )
